"""Sklearn SGD regressor fit callback atoms."""
...
from .witnesses import (
    witness_sgd_regressor_fit_c_value,
    witness_sgd_regressor_fit_callback_payload,
    witness_sgd_regressor_fit_more_validate_params_result,
    witness_sgd_regressor_fit_result,
)
...
@register_atom(witness_sgd_regressor_fit_more_validate_params_result)
@icontract.ensure(
    lambda result, validation_result: result is validation_result,
    "_more_validate_params callback result must be preserved",
)
def sgd_regressor_fit_more_validate_params_result(validation_result: object) -> object:
    """Return the unused result from BaseSGDRegressor._more_validate_params."""
    return validation_result


@register_atom(witness_sgd_regressor_fit_c_value)
@icontract.require(lambda alpha: _finite_nonnegative(alpha), "alpha must be finite and nonnegative")
@icontract.ensure(lambda result: result == 1.0, "BaseSGDRegressor.fit must pass fixed C=1.0")
def sgd_regressor_fit_c_value(alpha: float) -> float:
    """Return the fixed C value passed from BaseSGDRegressor.fit to _fit."""
    return 1.0


@register_atom(witness_sgd_regressor_fit_callback_payload)
...
@icontract.ensure(
    ...
    "fit payload must match the BaseSGDRegressor.fit _fit callback arguments",
)
def sgd_regressor_fit_callback_payload(
    X: object,
    y: object,
    *,
    alpha: float,
    ...
    """Return the _fit callback payload assembled by BaseSGDRegressor.fit."""
    return {
        "X": X,
        "y": y,
        "alpha": float(alpha),
        "C": sgd_regressor_fit_c_value(alpha),
        "loss": loss,
...
@register_atom(witness_sgd_regressor_fit_result)
...
@icontract.ensure(
    ...
    "BaseSGDRegressor.fit must return the delegated _fit result",
)
def sgd_regressor_fit_result(fit_result: object) -> object:
    """Return the object produced by the delegated BaseSGDRegressor._fit call."""
    return fit_result

"""Gaussian-process regression optimizer bookkeeping atoms adapted from scikit-learn."""
...
from .witnesses import (
    witness_gp_regression_restart_bounds,
    witness_gp_regression_restart_thetas,
    witness_gp_regression_select_best_optimum,
)
...
@register_atom(witness_gp_regression_restart_bounds)
@icontract.require(
...
def gp_regression_restart_bounds(
    bounds: NDArray[np.float64],
    *,
    n_restarts_optimizer: int = 0,
) -> NDArray[np.float64]:
    """Validate optimizer restart bounds the way GaussianProcessRegressor.fit does."""
...
@register_atom(witness_gp_regression_restart_thetas)
@icontract.require(
...
@icontract.ensure(
    ...
    "restart thetas must be finite with shape (n_restarts_optimizer, n_params)",
)
def gp_regression_restart_thetas(
    bounds: NDArray[np.float64],
    *,
    n_restarts_optimizer: int,
    random_state: RandomStateLike = 0,
) -> NDArray[np.float64]:
    """Draw GaussianProcessRegressor optimizer restart thetas from finite bounds."""
    values = np.asarray(bounds, dtype=np.float64)
    restart_count = int(n_restarts_optimizer)
    if restart_count == 0:
        return np.empty((0, values.shape[0]), dtype=np.float64)
    rng = check_random_state(random_state)
    draws = [rng.uniform(values[:, 0], values[:, 1]) for _ in range(restart_count)]
    return np.asarray(draws, dtype=np.float64)


@register_atom(witness_gp_regression_select_best_optimum)
@icontract.require(
...
def gp_regression_select_best_optimum(
    candidate_thetas: NDArray[np.float64],
    objective_values: NDArray[np.float64],
) -> tuple[NDArray[np.float64], float]:
    """Select the best GaussianProcessRegressor optimizer result from candidate objective values."""
    thetas = np.asarray(candidate_thetas, dtype=np.float64)
    objectives = np.asarray(objective_values, dtype=np.float64)
    best_index = int(np.argmin(objectives))

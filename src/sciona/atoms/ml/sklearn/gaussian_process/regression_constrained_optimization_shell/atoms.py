...
from .witnesses import (
    witness_gp_constrained_optimization_result,
    witness_gp_constrained_optimization_unknown_optimizer_message,
    witness_gp_constrained_optimization_use_callable,
    witness_gp_constrained_optimization_use_lbfgsb,
)
...
@register_atom(witness_gp_constrained_optimization_use_lbfgsb)
...
def gp_constrained_optimization_use_lbfgsb(
    optimizer_name: str,
) -> bool:
    """Decide whether GaussianProcessRegressor._constrained_optimization uses sklearn's L-BFGS-B branch."""
    return optimizer_name == "fmin_l_bfgs_b"


@register_atom(witness_gp_constrained_optimization_use_callable)
@icontract.require(lambda optimizer_is_callable: _bool(optimizer_is_callable), "optimizer_is_callable must be boolean")
@icontract.require(lambda optimizer_name: _nonempty_string(optimizer_name), "optimizer_name must be a nonempty string")
@icontract.ensure(lambda result: _bool(result), "result must be boolean")
def gp_constrained_optimization_use_callable(
    optimizer_is_callable: bool,
    optimizer_name: str,
) -> bool:
    """Decide whether GaussianProcessRegressor._constrained_optimization uses the user-callable optimizer branch."""
    return bool(not gp_constrained_optimization_use_lbfgsb(optimizer_name) and optimizer_is_callable)


@register_atom(witness_gp_constrained_optimization_unknown_optimizer_message)
@icontract.require(lambda optimizer_name: _nonempty_string(optimizer_name), "optimizer_name must be a nonempty string")
@icontract.ensure(
...
def gp_constrained_optimization_unknown_optimizer_message(
    optimizer_name: str,
) -> str:
...
@register_atom(witness_gp_constrained_optimization_result)
...
def gp_constrained_optimization_result(
    theta_opt: NDArray[np.float64],
    func_min: float,
) -> tuple[NDArray[np.float64], float]:
    """Package GaussianProcessRegressor._constrained_optimization's final return tuple."""

"""Functions for forest prediction preflight checks."""
...
from .witnesses import (
    witness_forest_predict_ensure_all_finite_mode,
    witness_forest_predict_require_sparse_int32_indices,
)
...
@register_atom(witness_forest_predict_ensure_all_finite_mode)
@icontract.require(
...
def forest_predict_ensure_all_finite_mode(
    supports_missing_values: bool,
) -> str | bool:
    """Return sklearn's ensure_all_finite mode for forest prediction validation."""
    return "allow-nan" if supports_missing_values else True


@register_atom(witness_forest_predict_require_sparse_int32_indices)
@icontract.require(
...
def forest_predict_require_sparse_int32_indices(
    indices: NDArray[np.integer],
    indptr: NDArray[np.integer],
) -> bool:
    """Enforce sklearn's CSR sparse-index dtype requirement before forest prediction."""

...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_cluster_optics_graph.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "cluster" / "cdg.json"
MANIFEST_PATH = ROOT / "data" / "audit_manifest.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.cluster.compute_optics_graph",
}
...
def test_bundle_level_fields() -> None:
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_cluster_optics_graph.review_bundle.json"
...
def test_cdg_atomic_nodes_have_publishable_io_specs() -> None:
    ...
    assert "compute_optics_graph" in {node["name"] for node in atomic}
    for node in atomic:
...
def test_manifest_contains_optics_graph_atom_after_merge() -> None:

...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_covariance.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "covariance" / "cdg.json"
...
def test_bundle_exists_and_has_two_atoms() -> None:
...
def test_bundle_level_fields() -> None:
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_covariance.review_bundle.json"
...
def test_cdg_atomic_nodes_have_publishable_io_specs() -> None:
    ...
    assert {node["name"] for node in atomic} == {"empirical_covariance", "shrunk_covariance"}
    for node in atomic:
...
def test_manifest_contains_covariance_atoms_after_merge() -> None:

...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_cluster_optics_xi.review_bundle.json"
...
def test_bundle_level_fields() -> None:
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_cluster_optics_xi.review_bundle.json"
...
def test_cdg_atomic_nodes_have_publishable_io_specs() -> None:
    ...
    assert "cluster_optics_xi" in {node["name"] for node in atomic}
    for node in atomic:
...
def test_manifest_contains_optics_xi_atom_after_merge() -> None:

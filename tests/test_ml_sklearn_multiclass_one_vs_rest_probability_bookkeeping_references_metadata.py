...
REFERENCES_PATH = Path("src/sciona/atoms/ml/sklearn/multiclass/one_vs_rest_probability_bookkeeping/references.json")
...
def test_one_vs_rest_probability_bookkeeping_references_cover_atoms() -> None:
    payload = json.loads(REFERENCES_PATH.read_text())
    found = {key.split("@", 1)[0] for key in payload["atoms"]}
    assert found == EXPECTED


def test_one_vs_rest_probability_bookkeeping_references_use_known_sources() -> None:
    payload = json.loads(REFERENCES_PATH.read_text())

...
REFERENCES_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "isotonic" / "references.json"
...
def test_references_json_exists_and_has_all_isotonic_fqdns() -> None:
...
def test_atom_leaf_names_are_registered() -> None:
    import_module("sciona.atoms.ml.sklearn.isotonic.atoms")
    registered = {name for name in REGISTRY if not name.startswith("witness_")}
    for fqdn in EXPECTED_FQDNS:
        leaf = fqdn.removeprefix("sciona.atoms.ml.sklearn.isotonic.")

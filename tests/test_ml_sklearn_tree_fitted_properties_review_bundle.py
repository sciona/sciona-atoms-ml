...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_tree_fitted_properties.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "tree" / "fitted_properties" / "cdg.json"
...
def test_bundle_exists_and_has_tree_fitted_properties_atoms() -> None:
    bundle = json.loads(BUNDLE_PATH.read_text())
    assert bundle["bundle_id"] == "sciona.atoms.review_bundle.ml.sklearn_tree_fitted_properties.v1"
    assert bundle["family_batch"] == "ml_sklearn_tree_fitted_properties"
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_tree_fitted_properties.review_bundle.json"

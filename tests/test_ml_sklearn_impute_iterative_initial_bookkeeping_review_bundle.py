...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_impute_iterative_initial_bookkeeping.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "impute" / "iterative_initial_bookkeeping" / "cdg.json"
MANIFEST_PATH = ROOT / "data" / "audit_manifest.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.impute.iterative_initial_bookkeeping.iterative_clear_empty_feature_missing_mask",
    "sciona.atoms.ml.sklearn.impute.iterative_initial_bookkeeping.iterative_empty_feature_mask",
    "sciona.atoms.ml.sklearn.impute.iterative_initial_bookkeeping.iterative_filter_nonempty_matrix",
    "sciona.atoms.ml.sklearn.impute.iterative_initial_bookkeeping.iterative_filter_nonempty_missing_mask",
    "sciona.atoms.ml.sklearn.impute.iterative_initial_bookkeeping.iterative_restore_empty_feature_imputations",
}
...
def test_bundle_exists_and_has_iterative_initial_bookkeeping_atoms() -> None:
...
def test_bundle_level_fields() -> None:
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_impute_iterative_initial_bookkeeping.review_bundle.json"
...
def test_manifest_contains_iterative_initial_bookkeeping_atoms_after_merge() -> None:

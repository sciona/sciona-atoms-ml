...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_manifold_tsne.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "manifold" / "tsne" / "cdg.json"
MANIFEST_PATH = ROOT / "data" / "audit_manifest.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.manifold.tsne.tsne_exact_joint_probabilities",
    "sciona.atoms.ml.sklearn.manifold.tsne.tsne_exact_kl_divergence",
    "sciona.atoms.ml.sklearn.manifold.tsne.tsne_gradient_descent_update",
}
...
def test_bundle_exists_and_has_tsne_atoms() -> None:
...
def test_bundle_level_fields() -> None:
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_manifold_tsne.review_bundle.json"
...
def test_manifest_contains_tsne_atoms_after_merge() -> None:

...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_feature_extraction_dict_vectorizer.review_bundle.json"
...
def test_dict_vectorizer_review_bundle_exists_and_parses() -> None:
...
def test_dict_vectorizer_review_bundle_contains_expected_atoms() -> None:
    payload = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))
    assert {row["atom_key"] for row in payload["rows"]} == EXPECTED_ATOMS


def test_dict_vectorizer_review_bundle_rows_are_publishable() -> None:
    payload = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))

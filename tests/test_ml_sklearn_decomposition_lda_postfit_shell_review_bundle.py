...
BUNDLE_PATH = Path("data/review_bundles/ml_sklearn_decomposition_lda_postfit_shell.review_bundle.json")
...
def test_lda_postfit_shell_review_bundle_rows_cover_atoms() -> None:
    payload = json.loads(BUNDLE_PATH.read_text())
    row_atoms = {row["atom_key"] for row in payload["rows"]}
    assert row_atoms == EXPECTED_ATOMS


def test_lda_postfit_shell_review_bundle_source_paths_are_family_local() -> None:
    payload = json.loads(BUNDLE_PATH.read_text())
    for row in payload["rows"]:
        for source_path in row["source_paths"]:
            assert source_path.startswith("src/sciona/atoms/ml/sklearn/decomposition/lda_postfit_shell/") or source_path == "tests/test_ml_sklearn_decomposition_lda_postfit_shell_behavior.py"


def test_lda_postfit_shell_review_bundle_is_reviewed() -> None:
    payload = json.loads(BUNDLE_PATH.read_text())

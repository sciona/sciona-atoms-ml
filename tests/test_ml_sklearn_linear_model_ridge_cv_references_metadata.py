...
EXPECTED_FQDNS = {
    "sciona.atoms.ml.sklearn.linear_model.ridge_cv_scores",
    "sciona.atoms.ml.sklearn.linear_model.ridge_cv_fit",
    "sciona.atoms.ml.sklearn.linear_model.ridge_cv_predict",
}


def test_references_json_exists_and_has_ridge_cv_fqdns() -> None:
    assert REFERENCES_PATH.exists()
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
    assert EXPECTED_FQDNS.issubset({key.partition("@")[0] for key in payload["atoms"]})


def test_ridge_cv_atoms_have_nonempty_references() -> None:
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
...
def test_ridge_cv_ref_ids_exist_in_local_registry() -> None:
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
...
def test_ridge_cv_reference_has_match_metadata() -> None:
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
    for key, entry in payload["atoms"].items():
        if key.partition("@")[0] in EXPECTED_FQDNS:
            for ref in entry["references"]:
...
def test_ridge_cv_atom_leaf_names_are_registered() -> None:

...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_kernel_approximation_chi2_samplers.review_bundle.json"
...
def test_chi2_sampler_review_bundle_exists_and_parses() -> None:
...
def test_chi2_sampler_review_bundle_contains_expected_atoms() -> None:
    payload = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))
    assert {row["atom_key"] for row in payload["rows"]} == EXPECTED_ATOMS


def test_chi2_sampler_review_bundle_rows_are_publishable() -> None:
    payload = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))

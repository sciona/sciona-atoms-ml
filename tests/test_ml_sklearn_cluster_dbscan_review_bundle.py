...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_cluster_dbscan.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "cluster" / "dbscan" / "cdg.json"
MANIFEST_PATH = ROOT / "data" / "audit_manifest.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.cluster.dbscan.dbscan_fit",
    "sciona.atoms.ml.sklearn.cluster.dbscan.dbscan_core_labels",
}
...
def test_bundle_exists_and_has_dbscan_atoms() -> None:
...
def test_bundle_level_fields() -> None:
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_cluster_dbscan.review_bundle.json"
...
def test_manifest_contains_dbscan_atoms_after_merge() -> None:

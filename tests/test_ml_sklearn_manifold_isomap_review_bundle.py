...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_manifold_isomap.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "manifold" / "cdg.json"
MANIFEST_PATH = ROOT / "data" / "audit_manifest.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.manifold.isomap_neighbors_graph",
    "sciona.atoms.ml.sklearn.manifold.isomap_geodesic_distances",
    "sciona.atoms.ml.sklearn.manifold.isomap_fit",
    "sciona.atoms.ml.sklearn.manifold.isomap_transform",
    "sciona.atoms.ml.sklearn.manifold.isomap_reconstruction_error",
}


def test_isomap_review_bundle_exists_and_parses() -> None:
...
def test_isomap_review_bundle_rows_are_publishable() -> None:
    bundle = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))
...
def test_isomap_cdg_nodes_have_io_specs() -> None:
    cdg = json.loads(CDG_PATH.read_text(encoding="utf-8"))
    nodes = {
        f"sciona.atoms.ml.sklearn.manifold.{node['name']}": node
...
def test_manifest_contains_isomap_atoms_after_merge() -> None:

...
FAMILY_DIR = Path("src/sciona/atoms/ml/sklearn/decomposition/lda_em_updates")
...
def test_lda_em_updates_references_cover_expected_atoms() -> None:
...
def test_lda_em_updates_references_use_known_ref_ids() -> None:
    payload = json.loads(REFERENCES_PATH.read_text())
...
def test_lda_em_updates_reference_locations_point_to_atoms_module() -> None:
    payload = json.loads(REFERENCES_PATH.read_text())
    for fqdn in payload["atoms"].keys():
        assert "@sciona/atoms/ml/sklearn/decomposition/lda_em_updates/atoms.py:" in fqdn

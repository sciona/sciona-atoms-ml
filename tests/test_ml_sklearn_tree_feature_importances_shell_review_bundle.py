...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_tree_feature_importances_shell.review_bundle.json"
CDG_PATH = (
    ROOT
    / "src"
    / "sciona"
...
def test_bundle_exists_and_has_tree_feature_importances_shell_atom() -> None:
    bundle = json.loads(BUNDLE_PATH.read_text())
    assert bundle["bundle_id"] == "sciona.atoms.review_bundle.ml.sklearn_tree_feature_importances_shell.v1"
    assert bundle["family_batch"] == "ml_sklearn_tree_feature_importances_shell"
    ...
    assert (
        ...
        == "data/review_bundles/ml_sklearn_tree_feature_importances_shell.review_bundle.json"
    )

...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_manifold_smacof_mds.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "manifold" / "cdg.json"
MANIFEST_PATH = ROOT / "data" / "audit_manifest.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.manifold.smacof",
    "sciona.atoms.ml.sklearn.manifold.mds_fit",
}


def test_smacof_mds_review_bundle_exists_and_parses() -> None:
...
def test_smacof_mds_review_bundle_rows_are_publishable() -> None:
    bundle = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))
...
def test_smacof_mds_cdg_nodes_have_io_specs() -> None:
...
def test_manifest_contains_smacof_mds_atoms_after_merge() -> None:

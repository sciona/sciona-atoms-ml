...
BUNDLE_PATH = (
    ...
    / "data"
    / "review_bundles"
    / "ml_sklearn_linear_model_coordinate_descent_estimator_loop_tail_shell.review_bundle.json"
)
CDG_PATH = (
    ROOT
    / "src"
...
def test_bundle_exists_and_has_coordinate_descent_estimator_loop_tail_shell_atoms() -> None:
    bundle = json.loads(BUNDLE_PATH.read_text())
    assert (
        bundle["bundle_id"]
        == "sciona.atoms.review_bundle.ml.sklearn_linear_model_coordinate_descent_estimator_loop_tail_shell.v1"
    )
    assert bundle["family_batch"] == "ml_sklearn_linear_model_coordinate_descent_estimator_loop_tail_shell"
    ...
    assert (
        ...
        == "data/review_bundles/ml_sklearn_linear_model_coordinate_descent_estimator_loop_tail_shell.review_bundle.json"
    )

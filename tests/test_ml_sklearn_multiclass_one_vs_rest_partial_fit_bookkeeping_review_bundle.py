...
BUNDLE_PATH = Path(
    "data/review_bundles/ml_sklearn_multiclass_one_vs_rest_partial_fit_bookkeeping.review_bundle.json"
)
...
def test_one_vs_rest_partial_fit_bookkeeping_review_bundle_rows_cover_atoms() -> None:
    payload = json.loads(BUNDLE_PATH.read_text())
    assert {row["atom_key"] for row in payload["rows"]} == EXPECTED


def test_one_vs_rest_partial_fit_bookkeeping_review_bundle_reviewed() -> None:
    payload = json.loads(BUNDLE_PATH.read_text())

...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_ensemble_forest_oob_postprocessing.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "ensemble" / "forest_oob_postprocessing" / "cdg.json"
MANIFEST_PATH = ROOT / "data" / "audit_manifest.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.ensemble.forest_oob_postprocessing.forest_classifier_oob_accuracy",
    "sciona.atoms.ml.sklearn.ensemble.forest_oob_postprocessing.forest_classifier_oob_decision_function",
    "sciona.atoms.ml.sklearn.ensemble.forest_oob_postprocessing.forest_regressor_oob_prediction",
    "sciona.atoms.ml.sklearn.ensemble.forest_oob_postprocessing.forest_regressor_oob_r2",
}
...
def test_bundle_exists_and_has_forest_oob_postprocessing_atoms() -> None:
...
def test_bundle_level_fields() -> None:
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_ensemble_forest_oob_postprocessing.review_bundle.json"
...
def test_manifest_contains_forest_oob_postprocessing_atoms_after_merge() -> None:

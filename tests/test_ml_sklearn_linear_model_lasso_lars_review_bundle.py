...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_linear_model_lasso_lars.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "linear_model" / "cdg.json"
MANIFEST_PATH = ROOT / "data" / "audit_manifest.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.linear_model.lasso_lars_path",
    "sciona.atoms.ml.sklearn.linear_model.lasso_lars_fit",
    "sciona.atoms.ml.sklearn.linear_model.lasso_lars_predict",
    "sciona.atoms.ml.sklearn.linear_model.lasso_lars_ic_fit",
    "sciona.atoms.ml.sklearn.linear_model.lasso_lars_ic_predict",
}
...
def test_bundle_exists_and_has_lasso_lars_atoms() -> None:
...
def test_bundle_level_fields() -> None:
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_linear_model_lasso_lars.review_bundle.json"
...
def test_cdg_atomic_nodes_have_publishable_io_specs() -> None:
    ...
    atom_names = {f"sciona.atoms.ml.sklearn.linear_model.{node['name']}" for node in atomic}
    assert EXPECTED_ATOM_NAMES.issubset(atom_names)
    for node in atomic:
        if f"sciona.atoms.ml.sklearn.linear_model.{node['name']}" not in EXPECTED_ATOM_NAMES:
            continue
...
def test_manifest_contains_lasso_lars_atoms_after_merge() -> None:

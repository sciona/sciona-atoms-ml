...
REFERENCES_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "manifold" / "tsne_fit_shell" / "references.json"
REGISTRY_PATH = ROOT / "data" / "references" / "registry.json"

EXPECTED_FQDNS = {
    "sciona.atoms.ml.sklearn.manifold.tsne_fit_shell.tsne_fit_return_self",
}


def test_tsne_fit_shell_references_json_has_expected_fqdns() -> None:
...
def test_tsne_fit_shell_ref_ids_exist_and_have_metadata() -> None:
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
...
def test_tsne_fit_shell_atom_leaf_name_is_registered() -> None:
    import_module("sciona.atoms.ml.sklearn.manifold.tsne_fit_shell.atoms")

...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_kernel_density.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "neighbors" / "cdg.json"
MANIFEST_PATH = ROOT / "data" / "audit_manifest.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.neighbors.kernel_density_fit",
    "sciona.atoms.ml.sklearn.neighbors.kernel_density_score_samples",
    "sciona.atoms.ml.sklearn.neighbors.kernel_density_score",
    "sciona.atoms.ml.sklearn.neighbors.kernel_density_sample",
}


def test_kernel_density_review_bundle_exists_and_parses() -> None:
...
def test_kernel_density_review_bundle_rows_are_publishable() -> None:
    bundle = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))
...
def test_kernel_density_cdg_nodes_have_io_specs() -> None:
...
def test_manifest_contains_kernel_density_atoms_after_merge() -> None:

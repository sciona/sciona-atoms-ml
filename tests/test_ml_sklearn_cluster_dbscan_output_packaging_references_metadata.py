...
REFERENCES_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "cluster" / "dbscan_output_packaging" / "references.json"
...
def test_dbscan_output_packaging_references_json_has_expected_fqdns() -> None:
...
def test_dbscan_output_packaging_ref_ids_exist_and_have_metadata() -> None:
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
...
def test_dbscan_output_packaging_atom_leaf_names_are_registered() -> None:
    import_module("sciona.atoms.ml.sklearn.cluster.dbscan_output_packaging.atoms")

...
REVIEW_BUNDLE = Path(
    "data/review_bundles/"
    "ml_sklearn_linear_model_coordinate_descent_path_validation_decorator_shell.review_bundle.json"
)
...
def test_coordinate_descent_path_validation_decorator_shell_review_bundle_shape() -> None:
    bundle = json.loads(REVIEW_BUNDLE.read_text())

    assert bundle["schema_version"] == "1.0"
    assert (
        bundle["bundle_id"]
        == "sciona.atoms.review_bundle.ml.sklearn_linear_model_coordinate_descent_path_validation_decorator_shell.v1"
    )
    assert bundle["provider_repo"] == "sciona-atoms-ml"
    assert bundle["family_batch"] == "ml_sklearn_linear_model_coordinate_descent_path_validation_decorator_shell"
...
def test_coordinate_descent_path_validation_decorator_shell_review_bundle_rows_are_catalog_ready() -> None:
    bundle = json.loads(REVIEW_BUNDLE.read_text())

...
FAMILY_DIR = Path("src/sciona/atoms/ml/sklearn/decomposition/sparse_encode_validation")
...
def test_sparse_encode_validation_references_cover_expected_atoms() -> None:
...
def test_sparse_encode_validation_references_use_known_ref_ids() -> None:
    payload = json.loads(REFERENCES_PATH.read_text())
...
def test_sparse_encode_validation_reference_locations_point_to_atoms_module() -> None:
    payload = json.loads(REFERENCES_PATH.read_text())
    for fqdn in payload["atoms"].keys():
        assert "@sciona/atoms/ml/sklearn/decomposition/sparse_encode_validation/atoms.py:" in fqdn

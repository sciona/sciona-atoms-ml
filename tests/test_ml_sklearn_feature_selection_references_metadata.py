...
REFERENCES_PATH = (
    ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "feature_selection" / "references.json"
)
...
def test_references_json_exists_and_has_all_feature_selection_fqdns() -> None:
    assert REFERENCES_PATH.exists()
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
    atom_keys = set(payload["atoms"])
    assert len(atom_keys) == 16

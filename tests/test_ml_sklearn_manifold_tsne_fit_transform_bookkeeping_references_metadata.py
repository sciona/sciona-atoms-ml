...
FAMILY_DIR = Path("src/sciona/atoms/ml/sklearn/manifold/tsne_fit_transform_bookkeeping")
...
def test_tsne_fit_transform_bookkeeping_references_cover_expected_atoms() -> None:
...
def test_tsne_fit_transform_bookkeeping_references_use_known_ref_ids() -> None:
    payload = json.loads(REFERENCES_PATH.read_text())
...
def test_tsne_fit_transform_bookkeeping_reference_locations_point_to_atoms_module() -> None:
    payload = json.loads(REFERENCES_PATH.read_text())
    for fqdn in payload["atoms"].keys():
        assert "@sciona/atoms/ml/sklearn/manifold/tsne_fit_transform_bookkeeping/atoms.py:" in fqdn

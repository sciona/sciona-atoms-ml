...
FAMILY = "coordinate_descent_cv_tags_super_callback_shell"
MODULE = f"sciona.atoms.ml.sklearn.linear_model.{FAMILY}"
REVIEW_BUNDLE = Path(
    "data/review_bundles/"
    "ml_sklearn_linear_model_coordinate_descent_cv_tags_super_callback_shell.review_bundle.json"
)

EXPECTED_ATOMS = {
    f"{MODULE}.cd_cv_tags_super_result",
    f"{MODULE}.cd_cv_tags_return",
}


def test_coordinate_descent_cv_tags_super_callback_shell_review_bundle_shape() -> None:
    bundle = json.loads(REVIEW_BUNDLE.read_text())

    assert bundle["schema_version"] == "1.0"
    assert (
        bundle["bundle_id"]
        == "sciona.atoms.review_bundle.ml.sklearn_linear_model_coordinate_descent_cv_tags_super_callback_shell.v1"
    )
    assert bundle["provider_repo"] == "sciona-atoms-ml"
    assert bundle["family_batch"] == "ml_sklearn_linear_model_coordinate_descent_cv_tags_super_callback_shell"
...
def test_coordinate_descent_cv_tags_super_callback_shell_review_bundle_rows_are_catalog_ready() -> None:
    bundle = json.loads(REVIEW_BUNDLE.read_text())

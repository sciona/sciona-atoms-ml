...
REFERENCES_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "neural_network" / "mlp_fit_bookkeeping" / "references.json"
...
def test_mlp_fit_bookkeeping_references_json_has_expected_fqdns() -> None:
...
def test_mlp_fit_bookkeeping_ref_ids_exist_and_have_metadata() -> None:
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
...
def test_mlp_fit_bookkeeping_atom_leaf_names_are_registered() -> None:
    import_module("sciona.atoms.ml.sklearn.neural_network.mlp_fit_bookkeeping.atoms")

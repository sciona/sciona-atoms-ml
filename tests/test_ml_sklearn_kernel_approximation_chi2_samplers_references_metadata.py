...
EXPECTED_FQDNS = {
    "sciona.atoms.ml.sklearn.kernel_approximation.skewed_chi2_sampler_fit",
    "sciona.atoms.ml.sklearn.kernel_approximation.skewed_chi2_sampler_transform",
    "sciona.atoms.ml.sklearn.kernel_approximation.additive_chi2_sampler_transform",
}


def test_references_json_exists_and_has_chi2_sampler_fqdns() -> None:
    assert REFERENCES_PATH.exists()
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
    assert EXPECTED_FQDNS.issubset({key.partition("@")[0] for key in payload["atoms"]})


def test_chi2_sampler_atoms_have_nonempty_references() -> None:
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
...
def test_chi2_sampler_ref_ids_exist_in_local_registry() -> None:
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
...
def test_chi2_sampler_reference_has_match_metadata() -> None:
    payload = json.loads(REFERENCES_PATH.read_text(encoding="utf-8"))
    for key, entry in payload["atoms"].items():
        if key.partition("@")[0] in EXPECTED_FQDNS:
            for ref in entry["references"]:
...
def test_chi2_sampler_atom_leaf_names_are_registered() -> None:

...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_svm_public_api_shell.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "svm" / "public_api_shell" / "cdg.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.svm.public_api_shell.svm_estimator_backend",
    "sciona.atoms.ml.sklearn.svm.public_api_shell.svm_estimator_catalog",
    "sciona.atoms.ml.sklearn.svm.public_api_shell.svm_estimator_methods",
    "sciona.atoms.ml.sklearn.svm.public_api_shell.svm_estimator_task",
    "sciona.atoms.ml.sklearn.svm.public_api_shell.svm_fit_return_self",
    "sciona.atoms.ml.sklearn.svm.public_api_shell.svm_liblinear_fitted_state",
    "sciona.atoms.ml.sklearn.svm.public_api_shell.svm_libsvm_fitted_support_state",
    "sciona.atoms.ml.sklearn.svm.public_api_shell.svm_linear_fit_liblinear_payload",
    "sciona.atoms.ml.sklearn.svm.public_api_shell.svm_prediction_method_payload",
}
...
def test_bundle_exists_and_has_svm_public_api_shell_atoms() -> None:
    assert BUNDLE_PATH.exists()
    bundle = _bundle()
    assert bundle["bundle_id"] == "sciona.atoms.review_bundle.ml.sklearn_svm_public_api_shell.v1"
    assert bundle["family_batch"] == "ml_sklearn_svm_public_api_shell"
...
def test_bundle_level_fields() -> None:
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_svm_public_api_shell.review_bundle.json"
    assert any("libsvm" in item and "liblinear" in item for item in bundle["limitations"])

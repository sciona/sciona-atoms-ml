...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_svm_helpers.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "svm" / "cdg.json"
MANIFEST_PATH = ROOT / "data" / "audit_manifest.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.svm.l1_min_c",
}
...
def test_bundle_level_fields() -> None:
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_svm_helpers.review_bundle.json"
...
def test_cdg_atomic_nodes_have_publishable_io_specs() -> None:
    ...
    assert {node["name"] for node in atomic} == {"l1_min_c"}
    for node in atomic:
...
def test_manifest_contains_l1_min_c_after_merge() -> None:

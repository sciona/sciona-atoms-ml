...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_tree_estimator_tags.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "tree" / "estimator_tags" / "cdg.json"
...
def test_bundle_exists_and_has_tree_estimator_tags_atoms() -> None:
    bundle = json.loads(BUNDLE_PATH.read_text())
    assert bundle["bundle_id"] == "sciona.atoms.review_bundle.ml.sklearn_tree_estimator_tags.v1"
    assert bundle["family_batch"] == "ml_sklearn_tree_estimator_tags"
    ...
    assert bundle["review_record_path"] == "data/review_bundles/ml_sklearn_tree_estimator_tags.review_bundle.json"

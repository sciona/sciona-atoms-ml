...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_manifold_spectral_embedding.review_bundle.json"
CDG_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "manifold" / "cdg.json"
MANIFEST_PATH = ROOT / "data" / "audit_manifest.json"

EXPECTED_ATOM_NAMES = {
    "sciona.atoms.ml.sklearn.manifold.spectral_embedding",
    "sciona.atoms.ml.sklearn.manifold.spectral_embedding_fit",
}


def test_spectral_embedding_review_bundle_exists_and_parses() -> None:
...
def test_spectral_embedding_review_bundle_rows_are_publishable() -> None:
    bundle = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))
...
def test_spectral_embedding_cdg_nodes_have_io_specs() -> None:
...
def test_manifest_contains_spectral_embedding_atoms_after_merge() -> None:

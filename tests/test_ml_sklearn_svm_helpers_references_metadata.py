...
REFERENCES_PATH = ROOT / "src" / "sciona" / "atoms" / "ml" / "sklearn" / "svm" / "references.json"
...
def test_references_json_exists_and_has_l1_min_c_fqdn() -> None:
...
def test_atom_leaf_name_is_registered() -> None:
    import_module("sciona.atoms.ml.sklearn.svm.atoms")
    registered = {name for name in REGISTRY if not name.startswith("witness_")}
    assert "l1_min_c" in registered

...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "ml_sklearn_decomposition_kernel_pca.review_bundle.json"
...
def test_kernel_pca_review_bundle_exists_and_parses() -> None:
...
def test_kernel_pca_review_bundle_contains_expected_atoms() -> None:
    payload = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))
    row_atoms = {row["atom_key"] for row in payload["rows"]}
    assert EXPECTED_ATOMS.issubset(row_atoms)


def test_kernel_pca_review_bundle_rows_are_publishable() -> None:
    payload = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))
    for row in payload["rows"]:
        if row["atom_key"] not in EXPECTED_ATOMS:
            continue
